...
async function showSettings(player) {
  ...
  try {
    ...
    const form = new CustomForm(player, "VoiceCraft • Mic Settings")
      ...
      .divider()
      .header("Voice Range")
      .label("กำหนดระยะที่ผู้เล่นอื่นจะได้ยินเสียงของคุณ")
      .spacer()
      ...
      .slider(
        ...
        {
          step: 1,
          description: "เลื่อนเพื่อเลือกระยะอย่างรวดเร็ว แล้วกดใช้ระยะจาก Slider",
        }
      )
...
'''
    ...
    js = js.replace(
        ...
        "[VoiceCraftItem/BP] Loaded v2.5.3 — DDUI slider + advanced settings + range confirmation",
        1,
    )
    ...
    if missing:
        raise RuntimeError(f"Item Mic 2.5.3 DDUI patch failed: {missing}")
    ...
    if leftovers:
        raise RuntimeError(f"Unexpected API remains in Item Mic 2.5.3: {leftovers}")
...
def main() -> None:
    ...
    output = Path(
        sys.argv[2]
        if len(sys.argv) > 2
        else "release-assets/VoiceCraft_ItemMic_v2.5.3_DDUI.mcaddon"
    )
    ...
    files["BP/manifest.json"] = patch_manifest(
        ...
        "VoiceCraft Item Mic BP v2.5.3 DDUI",
        "Stable DDUI Mic settings with Voice Range slider, advanced controls, server confirmation and reset.",
    )
    files["RP/manifest.json"] = patch_manifest(
        files["RP/manifest.json"],
        "VoiceCraft Mic Icons RP v2.5.3",
        "Inventory icons plus invisible held Mic model for VoiceCraft Item Mic v2.5.3 DDUI.",
    )
    ...
    with tempfile.TemporaryDirectory(prefix="voicecraft-itemmic-2.5.3-ddui-") as td:
        tmp = Path(td)
        bp = tmp / "BP"
        rp = tmp / "RP"
        ...
        rp_pack = tmp / "VoiceCraft_ItemMic_RP_v2.5.3.mcpack"
        bp_pack = tmp / "VoiceCraft_ItemMic_BP_v2.5.3.mcpack"
