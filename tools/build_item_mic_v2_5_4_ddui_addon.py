...
async function showSettings(player) {
  ...
  try {
    ...
    const form = new CustomForm(player, "VoiceCraft • Mic Settings")
      ...
      .divider()
      .header("Mic Mode")
      .label("§7เลือกรูปแบบการเปิดและปิดไมค์§r")
      .spacer()
      .label("§7Hold-to-Talk — ถือ Mic เพื่อเปิด / เลิกถือหรือเปลี่ยนช่องเพื่อปิด§r")
      .button("Hold-to-Talk", () => {
        applyMicModeFromUi(
          player,
          MODE_HOLD,
      ...
      .spacer()
      .label("§7Toggle — ถือ Mic หนึ่งครั้งเพื่อเปิด / ถือ Mic อีกครั้งเพื่อปิด§r")
      .button("Toggle", () => {
        applyMicModeFromUi(
          player,
          MODE_TOGGLE,
      ...
      .divider()
      .header("Reset")
      .label("§7คืนค่า Mic Mode เป็น Hold-to-Talk และ Voice Range เป็น 20 บล็อก§r")
...
'''
    ...
    js = js.replace(
        ...
        "[VoiceCraftItem/BP] Loaded v2.5.4 — DDUI description polish + slider + advanced settings + range confirmation",
        1,
    )
    ...
    required = [
        ...
        'โหมด: §e${modeUiLabel(initial.mode)}§r',
        'ระยะเสียงปัจจุบัน: §b${initialRange} บล็อก§r',
        "§7เลือกรูปแบบการเปิดและปิดไมค์§r",
        "§7Hold-to-Talk — ถือ Mic เพื่อเปิด / เลิกถือหรือเปลี่ยนช่องเพื่อปิด§r",
        "§7Toggle — ถือ Mic หนึ่งครั้งเพื่อเปิด / ถือ Mic อีกครั้งเพื่อปิด§r",
        "กำหนดระยะที่ผู้เล่นอื่นจะได้ยินเสียงของคุณ • เลื่อนเพื่อเลือกระยะอย่างรวดเร็ว",
        "§7คืนค่า Mic Mode เป็น Hold-to-Talk และ Voice Range เป็น 20 บล็อก§r",
    ...
    if missing:
        raise RuntimeError(f"Item Mic 2.5.4 DDUI patch failed: {missing}")
    ...
    if leftovers:
        raise RuntimeError(f"Unexpected API remains in Item Mic 2.5.4: {leftovers}")
...
def main() -> None:
    ...
    output = Path(
        sys.argv[2]
        if len(sys.argv) > 2
        else "release-assets/VoiceCraft_ItemMic_v2.5.4_DDUI.mcaddon"
    )
    ...
    files["BP/manifest.json"] = patch_manifest(
        ...
        "VoiceCraft Item Mic BP v2.5.4 DDUI",
        "Stable DDUI Mic settings with compact descriptions, Voice Range slider, advanced controls, server confirmation and reset.",
    )
    files["RP/manifest.json"] = patch_manifest(
        files["RP/manifest.json"],
        "VoiceCraft Mic Icons RP v2.5.4",
        "Inventory icons plus invisible held Mic model for VoiceCraft Item Mic v2.5.4 DDUI.",
    )
    ...
    with tempfile.TemporaryDirectory(prefix="voicecraft-itemmic-2.5.4-ddui-") as td:
        tmp = Path(td)
        bp = tmp / "BP"
        rp = tmp / "RP"
        ...
        rp_pack = tmp / "VoiceCraft_ItemMic_RP_v2.5.4.mcpack"
        bp_pack = tmp / "VoiceCraft_ItemMic_BP_v2.5.4.mcpack"
