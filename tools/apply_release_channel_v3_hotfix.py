...
def patch_installer(path: Path) -> None:
    ...
    if desired_plugin not in text:
        ...
        text, count = plugin_pattern.subn(desired_plugin, text, count=1)
        if count != 1:
            raise RuntimeError("could not resolve MCSV PluginWheelUrl to latest release")

    desired_item = (
        '    private const string ItemMicUrl =\n'
        f'        "{REPO_RELEASE_BASE}{ADDON_NAME}";'
    )
    if desired_item not in text:
        item_pattern = re.compile(
            r'    private const string ItemMicUrl =\n'
            r'        "https://github\.com/samsosleepy2007/VoiceCraft-Server-Mobile-Unofficial/releases/download/" \+\n'
            r'        "[^"]+/VoiceCraft_ItemMic_v2\.4\.0\.mcaddon";'
        )
        text, count = item_pattern.subn(desired_item, text, count=1)
        if count != 1:
            raise RuntimeError("could not resolve MCSV ItemMicUrl to latest release")
...
def patch_configured_downloader(path: Path) -> None:
    ...
    old_name = '    private const string WheelFileName = "endstone_voicecraft-0.2.6-py3-none-any.whl";'
    new_name = f'    private const string WheelFileName = "{PLUGIN_NAME}";'
    if new_name not in text:
        if old_name not in text:
            raise RuntimeError("could not resolve configured plugin wheel version")
        text = text.replace(old_name, new_name, 1)

    old_sha = '    private const string SourceWheelSha256 = "b6725cc94609d27b3d2815f66e1373fe8c6917d7be68562456ac7727254e8ae4";'
    new_sha = f'    private const string SourceWheelSha256 = "{PLUGIN_SHA256}";'
    if new_sha not in text:
        if old_sha not in text:
            raise RuntimeError("could not resolve configured plugin SHA-256")
    ...
    if desired_url not in text:
        ...
        text, count = url_pattern.subn(desired_url, text, count=1)
        if count != 1:
            raise RuntimeError("could not resolve configured plugin SourceWheelUrl to latest release")
...
def main() -> None:
    ...
    required = (REPO_RELEASE_BASE, PLUGIN_NAME, ADDON_NAME)
    ...
    print("Applied/verified VoiceCraft MCSV V3.0.1 latest-release channel hotfix")
