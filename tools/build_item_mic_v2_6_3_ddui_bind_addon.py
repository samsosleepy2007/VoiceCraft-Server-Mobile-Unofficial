...
def patch_script(js: str) -> str:
    ...
    if anchor not in js:
        raise RuntimeError("Item Mic 2.6.3 bind DDUI anchor missing")
...
function requestBindUi(player, source = "addon") {
  if (openBindPlayers.has(player.id)) return false;

  const now = system.currentTick;
  const nextAllowed = bindRequestAfterTick.get(player.id) ?? 0;
  if (now < nextAllowed) return false;

  removeTagsByPrefix(player, BIND_UI_REQUEST_PREFIX);
  try {
    player.addTag(BIND_UI_REQUEST_PREFIX + source);
    bindRequestAfterTick.set(player.id, now + 20);
    return true;
  } catch (e) {
    console.warn("[VoiceCraftItem/BP] bind UI request failed player=" + player.name + ": " + e);
    return false;
  }
}
...
function handleMicUse(player) {
  ...
  if (state === "reconnecting") {
    player.sendMessage("§e[VoiceCraft] กำลังเชื่อมต่อ VoiceCraft กลับอัตโนมัติ กรุณารอสักครู่...§r");
    return;
  }
  ...
  try {
    ...
    if (stateAfterClose !== "bound") {
      if (reasonName.includes("UserBusy")) {
        ...
        bindRequestAfterTick.delete(player.id);
      } else if (
...
world.afterEvents.playerLeave.subscribe((ev) => {
  ...
  bindRequestAfterTick.delete(ev.playerId);
});
  ...
  for (const player of world.getAllPlayers()) {
    try {
      ...
      if (
        ...
        !openBindPlayers.has(player.id)
      ) {
        const token = getTagValue(player, BIND_OPEN_PREFIX);
        const lastToken = bindLastOpenToken.get(player.id) ?? "";
        if (!token || token === lastToken) {
          requestBindUi(player, "rebind_watchdog");
        }
      }
    } catch (e) {
...
'''
    ...
    js = js.replace(
        ...
        "[VoiceCraftItem/BP] Loaded v2.6.3 — unified Bind DDUI requests + Mic-to-Bind routing",
        1,
    )
    ...
    if missing:
        raise RuntimeError(f"Item Mic 2.6.3 bind DDUI patch failed: {missing}")
    ...
    if leftovers:
        raise RuntimeError(f"Unexpected legacy/preview UI in Item Mic 2.6.3: {leftovers}")
...
def main() -> None:
    ...
    output = Path(
        sys.argv[2]
        if len(sys.argv) > 2
        else "release-assets/VoiceCraft_ItemMic_v2.6.3_DDUI-Bind.mcaddon"
    )
    ...
    files["BP/manifest.json"] = patch_manifest(
        ...
        "VoiceCraft Item Mic BP v2.6.3 DDUI Bind",
        "Stable Mic Settings DDUI plus unified Endstone Binding Key requests, reconnect/rebind lifecycle and Mic-to-Bind routing.",
    )
    files["RP/manifest.json"] = patch_manifest(
        files["RP/manifest.json"],
        "VoiceCraft Mic Icons RP v2.6.3",
        "Inventory icons plus invisible held Mic model for VoiceCraft Item Mic v2.6.3.",
    )
    ...
    with tempfile.TemporaryDirectory(prefix="voicecraft-itemmic-2.6.3-ddui-bind-") as td:
        tmp = Path(td)
        bp = tmp / "BP"
        rp = tmp / "RP"
        ...
        rp_pack = tmp / "VoiceCraft_ItemMic_RP_v2.6.3.mcpack"
        bp_pack = tmp / "VoiceCraft_ItemMic_BP_v2.6.3.mcpack"
