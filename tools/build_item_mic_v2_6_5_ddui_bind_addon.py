...
def patch_script(js: str) -> str:
    ...
    if anchor not in js:
        raise RuntimeError("Item Mic 2.6.5 bind DDUI anchor missing")
...
function openBindFromMic(player) {
  ...
  // Mic is an explicit user action. Open the Bind DDUI locally instead of
  // round-tripping through Endstone for another open token.
  removeTagsByPrefix(player, BIND_OPEN_PREFIX);
  bindLastOpenToken.delete(player.id);
  bindRetryAfterTick.delete(player.id);

  const localToken = "mic_" + String(system.currentTick);
  showBindDdui(player, localToken);
}
...
function submitBindingKey(player, rawValue, statusText) {
  ...
  try {
    ...
    statusText.setData("สถานะ: §eกำลังเชื่อมต่อไมโครโฟน...§r");
    return true;
  } catch (e) {
...
async function showBindDdui(player, openToken) {
  ...
  try {
    ...
    const submitVisible = new ObservableBoolean(true);
    const inputDisabled = new ObservableBoolean(false);
    ...
    const unlockSubmit = () => {
      ...
      submitStartedTick = 0;
      submitVisible.setData(true);
      inputDisabled.setData(false);
    };
    ...
    const form = new CustomForm(player, "VoiceCraft • BIND")
      ...
      .button("เชื่อมต่อไมโครโฟน", () => {
        ...
        // Lock synchronously before touching the server tag. Even if the
        // client renders the old button for another frame, repeat clicks are
        // ignored locally and cannot enqueue additional Bind attempts.
        submitInFlight = true;
        submitStartedTick = system.currentTick;
        submitVisible.setData(false);
        inputDisabled.setData(true);

        const queued = submitBindingKey(
          player,
          bindingKey.getData(),
          statusText
        );
    ...
    refreshId = system.runInterval(() => {
      try {
        ...
        if (state === "bound") {
          ...
          // Do not gate close() behind isShowing(). close() is the authority;
          // if the screen is between visibility states it may throw, so retry
          // on the next refresh instead of leaving a successful Bind open.
          if (boundCloseAttempts < 20) {
            boundCloseAttempts++;
            try {
              form.close();
            } catch (e) {
              if (boundCloseAttempts === 20) {
                console.warn(
                  "[VoiceCraftItem/BP] bind DDUI close failed after retries player=" +
                  player.name + ": " + e
                );
              }
            }
        ...
        // A missing/unavailable Endstone bridge must not leave the user
        // permanently locked out of the form. Treat 10 seconds without a
        // pending/bound/error transition as a local connection error.
        if (
          submitInFlight &&
          submitStartedTick > 0 &&
          system.currentTick - submitStartedTick >= 200
        ) {
          removeTagsByPrefix(player, BIND_REQUEST_PREFIX);
          statusText.setData(
            "สถานะ: §cเซิร์ฟเวอร์ไม่ตอบกลับ กรุณาตรวจสอบ VoiceCraft แล้วลองใหม่§r"
          );
          unlockSubmit();
        }
      } catch {}
...
'''
    ...
    mic_use_old = """    onUse(arg) {
      const player = arg.source;
      system.run(() => showSettings(player));
    },"""
    mic_use_new = """    onUse(arg) {
      const player = arg.source;
      system.run(() => handleMicUse(player));
    },"""
    if mic_use_old not in js:
        raise RuntimeError("Item Mic 2.6.3 Mic-use routing anchor missing")
    ...
    required = [
        ...
        "disabled: inputDisabled",
        "system.currentTick - submitStartedTick >= 200",
        "เซิร์ฟเวอร์ไม่ตอบกลับ กรุณาตรวจสอบ VoiceCraft แล้วลองใหม่",
        "VoiceCraft • BIND",
        '.textField(\n        "",',
    ...
    if missing:
        raise RuntimeError(f"Item Mic 2.6.5 bind DDUI patch failed: {missing}")
    ...
    forbidden = [
        ...
        'requestBindUi(player, "mic_item")',
        '"rebind_watchdog"',
    ]
    leftovers = [value for value in forbidden if value in js]
    if leftovers:
        raise RuntimeError(f"Unexpected legacy/preview UI in Item Mic 2.6.5: {leftovers}")
...
def main() -> None:
    ...
    output = Path(
        sys.argv[2]
        if len(sys.argv) > 2
        else "release-assets/VoiceCraft_ItemMic_v2.6.5_DDUI-Bind.mcaddon"
    )
    ...
    files["BP/manifest.json"] = patch_manifest(
        ...
        "VoiceCraft Item Mic BP v2.6.5 DDUI Bind",
        "Stable Mic Settings DDUI plus single-submit Bind protection, reliable success close, direct Mic-to-Bind and Endstone Binding Key backend.",
    )
    files["RP/manifest.json"] = patch_manifest(
        files["RP/manifest.json"],
        "VoiceCraft Mic Icons RP v2.6.5",
        "Inventory icons plus invisible held Mic model for VoiceCraft Item Mic v2.6.5.",
    )
    ...
    with tempfile.TemporaryDirectory(prefix="voicecraft-itemmic-2.6.5-ddui-bind-") as td:
        tmp = Path(td)
        bp = tmp / "BP"
        rp = tmp / "RP"
        ...
        rp_pack = tmp / "VoiceCraft_ItemMic_RP_v2.6.5.mcpack"
        bp_pack = tmp / "VoiceCraft_ItemMic_BP_v2.6.5.mcpack"
