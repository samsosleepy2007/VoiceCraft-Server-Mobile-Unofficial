...
def patch_script(js: str) -> str:
    ...
    if anchor not in js:
        raise RuntimeError("Item Mic 2.6.0 bind DDUI anchor missing")
...
async function showBindDdui(player) {
  ...
  try {
    ...
    const form = new CustomForm(player, "VoiceCraft • เชื่อมต่อไมโครโฟน")
      .label(statusText)
      .spacer()
      .header("Binding Key")
      .textField(
      ...
      .spacer()
      .label("ถ้า VoiceCraft หลุด ระบบจะรอเชื่อมต่อกลับอัตโนมัติก่อน และจะแจ้งให้กรอก Key ใหม่เมื่อจำเป็น")
      .spacer()
      .closeButton();
...
function pollBindDdui(player) {
  ...
  system.run(() => showBindDdui(player));
}
...
'''
    ...
    js = js.replace(
        ...
        "[VoiceCraftItem/BP] Loaded v2.6.0 — stable Mic DDUI + Endstone Binding Key DDUI",
        1,
    )
    ...
    if missing:
        raise RuntimeError(f"Item Mic 2.6.0 bind DDUI patch failed: {missing}")
    ...
    if leftovers:
        raise RuntimeError(f"Unexpected legacy/preview UI in Item Mic 2.6.0: {leftovers}")
...
def main() -> None:
    ...
    output = Path(
        sys.argv[2]
        if len(sys.argv) > 2
        else "release-assets/VoiceCraft_ItemMic_v2.6.0_DDUI-Bind.mcaddon"
    )
    ...
    files["BP/manifest.json"] = patch_manifest(
        ...
        "VoiceCraft Item Mic BP v2.6.0 DDUI Bind",
        "Stable Mic Settings DDUI plus Endstone Binding Key DDUI and reconnect/rebind status.",
    )
    files["RP/manifest.json"] = patch_manifest(
        files["RP/manifest.json"],
        "VoiceCraft Mic Icons RP v2.6.0",
        "Inventory icons plus invisible held Mic model for VoiceCraft Item Mic v2.6.0.",
    )
    ...
    with tempfile.TemporaryDirectory(prefix="voicecraft-itemmic-2.6.0-ddui-bind-") as td:
        tmp = Path(td)
        bp = tmp / "BP"
        rp = tmp / "RP"
        ...
        rp_pack = tmp / "VoiceCraft_ItemMic_RP_v2.6.0.mcpack"
        bp_pack = tmp / "VoiceCraft_ItemMic_BP_v2.6.0.mcpack"
